...
test.mergeTFmodelWithRegulatoryRegions <- function()
{
   ...
   load("../skin-PnG/tbl.fp.380x17.forTesting.RData")
   load("../skin-PnG/tbl.model.7x9.forTesting.RData")
   load("../skin-PnG/tbl.fptf.5789.forTesting.RData")
   load("../skin-PnG/tbl.model.7x9.VGF.RData")
   target.gene <- "VGF"
...
test.createGeneModel <- function()
{
   ...
   checkEquals(names(which(table(tbl.reg$name) > 1)), "MA0786.1")
...
test.createGeneModelMultipleRegions <- function()
{
   ...
   region <- "5:88,904,000-88,909,000"
   target.gene <- "MEF2C"
   result.1 <- createGeneModel(mtx=mtx.rosmap, target.gene, regions=region)

   region.a <- "5:88,904,000-88,907,000"
   region.b <- "5:88,907,000-88,909,000"
   result.2 <- createGeneModel(mtx=mtx.rosmap, target.gene, regions=c(region.a, region.b))

   checkEquals(result.1$msg, result.2$msg)

   tbl.gm1 <- result.1$model
   tbl.gm2 <- result.2$model

     # though stochasticity produces slightly different results in every model created
     # created from identical data, the gross structure of the results should be
     # the same:

   rf.genes.1 <- tbl.gm1[order(tbl.gm1$rf.score, decreasing=TRUE), "gene"]
   rf.genes.2 <- tbl.gm2[order(tbl.gm2$rf.score, decreasing=TRUE), "gene"]
   checkTrue(all(rf.genes.1 %in% rf.genes.2))
   checkTrue(all(rf.genes.2 %in% rf.genes.1))

   tbl.reg1 <- result.1$regulatoryRegions
   tbl.reg2 <- result.2$regulatoryRegions

   checkTrue(all(tbl.reg1$name %in% tbl.reg2$name))
...
test.tablesToFullGraph <- function()
{
   ...
   load("../skin-PnG/geneModel.COL1A1.6motifs.10genes.RData")
